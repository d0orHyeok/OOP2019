...
public class TurtleGraphics {
	...
	public TurtleGraphics() {
		...
		System.out.printf("Type command%n1:Pen up%n2:Pen down%n3:Turn Right%n4:Turn left"
				...
				+ "%n7,10 Move backward 10 spaces (replace 10 for a different number of spaces)"
	...
	private void type_command() {
		...
		if(typecommand.length() != 1) { // command 값이 ,로 2개가 들어올때 
	...
	private void moveTurtle(boolean fb) { // fb는 앞으로 가는지 뒤로가는지 확인을 위해서 
		...
		if(fb == true) // move 일떄
			...
			direction = this.direction+2%4; // 방향을 반대로 바꿈
